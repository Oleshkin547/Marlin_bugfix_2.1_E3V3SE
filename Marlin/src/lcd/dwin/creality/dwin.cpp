...
void _update_axis_value(const AxisEnum axis, const uint16_t x, const uint16_t y, const bool blink, const bool force)
{
  ...
  float p;
  if (axis == Z_AXIS)
    p = probe.offset.z;           // Z offset вместо текущей высоты
  else
    p = motion.position[axis];
  ...
  if (force || changed || draw_qmark || draw_empty)
  {
    ...
    {
      if (axis == Z_AXIS)
        // Для Z offset: два знака после запятой, значение умножаем на 100
        DWIN_Draw_Signed_Float(font8x16, Color_Bg_Black, 2, 2, x, y, p *100);
      else
        // Для X, Y: один знак после запятой, значение умножаем на 10
...
void HMI_Levling_Change()
{
  ...
  else
  {
    ...
    else
    {
      ...
      // DO_BLOCKING_MOVE_TO_Z(dwin_zoffset_edit + 10.0f, 5);
      
      dwin_zoffset = dwin_zoffset_edit; // Приравниваем экранное значение к редактируемому
      probe.offset.z = dwin_zoffset;    // Сразу записываем в настройки датчика
